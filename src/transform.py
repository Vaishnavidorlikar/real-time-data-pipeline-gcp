...
class EventTransformer:
    ...
    def __init__(self):
        ...
        self.event_type_mapping = {
            ...
            # producer's actual event_type values
            'click': 'CLICK',
    ...
    def transform_event(self, event: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        Args:
            event: Raw (flat) event data from Pub/Sub, matching the shape
                   produced by PubSubProducer.generate_sample_event /
        ...
        try:
            ...
            # processing_timestamp may not be set yet (producer doesn't set it) -
            # default to "now" if missing, since that's when we're processing it.
            processing_ts = event.get('processing_timestamp')
            if processing_ts:
                transformed['processing_timestamp'] = self._parse_timestamp(processing_ts)
            else:
            ...
            # FIXED: these are top-level fields in the actual producer output,
            # not nested under event['data'] / event['data']['metadata'].
            transformed['action'] = self._normalize_action(event.get('action'))
            transformed['value'] = self._clean_numeric(event.get('value'))

            transformed['source'] = self._clean_string(event.get('source', 'realtime_pipeline'))
            transformed['version'] = self._clean_string(event.get('version', '1.0'))
            transformed['environment'] = self._normalize_environment(event.get('environment'))
            ...
            transformed['processing_status'] = self._clean_string(
            ...
            if self._validate_transformed_event(transformed):
                ...
                self.logger.warning(f"Invalid transformed event: {transformed.get('event_id')}")
                return None
    ...
    def _normalize_environment(self, environment: Optional[str]) -> str:
    ...
    def _extract_partition_date(self, timestamp: Optional[str]) -> Optional[str]:
    ...
    def _calculate_latency(
        self, event_timestamp: Optional[str], processing_timestamp: Optional[str]
    ) -> Optional[int]:
